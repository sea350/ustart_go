...
func PrototypeUserSearchScroll(eclient *elastic.Client, searchTerm string, sortBy int, searchBy []bool, mustMajor []string, mustTag []string, mustLoc []types.LocStruct, scrollID string) (int, string, []types.FloatingHead, error) {
	...
	query = query.MustNot(elastic.NewTermQuery("Status", false))
	...
	if len(mustMajor) > 0 {
		for _, element := range mustMajor {
			...
			// query = query.Must(elastic.NewMatchQuery("Major", strings.ToLower(element)))
			query = query.Should(elastic.NewFuzzyQuery("Majors.keyword", strings.ToLower(element)).Fuzziness("AUTO"))
		}
	}

	if len(mustTag) > 0 {
		for _, tag := range mustTag {
			query = query.Must(elastic.NewMatchQuery("Tags", strings.ToLower(tag)))
		}
	}
