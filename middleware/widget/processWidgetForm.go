...
func ProcessWidgetForm(r *http.Request) (types.Widget, error) {
	...
	if r.FormValue("widgetSubmit") == `7` {
		//codepen -- Embed code
		regX := regexp.MustCompile(`<iframe height='[0-9%]{0,4}' scrolling='no' title='(.*?)' src='\/\/codepen\.io\/(.*?)\/embed\/(.*?)\/?height=[0-9%]{0,4}&theme-id=[0-9%]{0,4}&default-tab=(.*?),result&embed-version=2' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>See the Pen <a href='https:\/\/codepen.io\/(.*?)\/pen\/(.*?)\/'>(.*?)<\/a> by (.*?)\(<a href='https:\/\/codepen.io\/(.*?)'>@(.*?)<\/a>\) on <a href='https:\/\/codepen.io'>CodePen<\/a>. <\/iframe>`)
